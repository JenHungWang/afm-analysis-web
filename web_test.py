        ...
        if cno < 5:
            ...
            for j in range(len(levels) - 1):
                ...
                if layer_area == 0:
                    ...
                    density = np.round(
                        (ecno / layer_area) * result.orig_img.shape[0] * result.orig_img.shape[1] / (img_h * img_w), 4)
...
with gr.Blocks(title="AFM AI Analysis", theme="default") as app:
        ...
        with gr.Column():
            ...
            with gr.Accordion("User Information", open=True):
                ...
                with gr.Row():
                    ...
                    age_slider = gr.Slider(minimum=0, maximum=100, step=1, value=0, label="Age", interactive=True,
                                           scale=2)
                with gr.Group():
                    fitzpatrick = gr.Slider(minimum=1, maximum=6, step=1, value=1, label="Fitzpatrick",
                                            interactive=True)
                history = gr.Checkboxgroup(["Familial Disease", "Allergic Rhinitis", "Asthma"], label="Medical History",
                                           interactive=True)

            input_files = gr.File(file_types=["image"], file_count="multiple", label="Upload Image")
            # gr.Markdown("Model Configuration")
            with gr.Accordion("Model Configuration", open=False):
                model_radio = gr.Radio(["YOLOv8-N", "YOLOv8-S", "YOLOv8-M", "YOLOv8-L", "YOLOv8-X"],
                                       label="Model Selection", value="YOLOv8-M")
    ...
    clear_btn.click(reset,
                    outputs=[name_textbox, img_h, img_w, gender_radio, age_slider, fitzpatrick, history, model_radio,
